...
function initVote() {
  ...
  if (localStorage.getItem('voted')) {
    ...
    /* 선택했던 항목만 강조 */
    const votedKey = localStorage.getItem('voted');
    const votedBtn = document.querySelector(`[data-vote="${votedKey}"]`);
    if (votedBtn) votedBtn.style.opacity = '1';
  ...
  btns.forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        /* ★ localStorage에 저장 */
        localStorage.setItem('voted', key);
...
function initVoiceInput() {
  ...
  const MAX       = 60;
  const MAX_POSTS = 3;           /* 24시간 최대 게시 횟수 */
  ...
  function getRemainingPosts() {
    const now     = Date.now();
    const history = getPostHistory().filter(t => now - t < WINDOW_MS);
    localStorage.setItem('voice_history', JSON.stringify(history));
    return MAX_POSTS - history.length;
  }
  ...
  function updateSubmitState() {
    ...
    if (remaining <= 0) {
      ...
      submit.textContent = '오늘 게시 한도 도달 (내일 초기화)';
      input.disabled     = true;
      input.placeholder  = '오늘은 더 이상 게시할 수 없습니다.';
    } else {
      submit.textContent = `목소리 남기기 (오늘 ${remaining}회 남음)`;
    }
  }
